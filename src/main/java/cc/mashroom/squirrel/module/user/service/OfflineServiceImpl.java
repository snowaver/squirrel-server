...
public  class  OfflineServiceImpl  implements  OfflineService
{
	...
	public  ResponseEntity<String>  lookup( int  action,long  userId,Map<String,Map<String,Object>>  extras )
	{
		...
		response.addEntry( "OFFLINE_CHAT_MESSAGES",OfflineChatMessageRepository.DAO.lookup(Map.class,"SELECT  ID,USER_ID  AS  CONTACT_ID,MD5,CONTENT_TYPE,CONTENT  FROM  "+OfflineChatMessageRepository.DAO.getDataSourceBind().table()+"  WHERE  CONTACT_ID = ?  AND  ID > ?  ORDER  BY  ID  ASC",new  Object[]{userId,extras.get("OFFLINE_CHAT_MESSAGES").getLong("LATEST_RECEIVED_ID")}).stream().filter((chatMessage) -> {chatMessage.addEntry("CREATE_TIME",new  Timestamp(chatMessage.getLong("ID"))).addEntry("TRANSPORT_STATE",TransportState.RECEIVED.getValue());  return  true;}).collect(Collectors.toList()) );
		
		response.addEntry( "OFFLINE_GROUP_CHAT_MESSAGES",OfflineGroupChatMessageRepository.DAO.lookup(Map.class,"SELECT  ID,GROUP_ID,USER_ID  AS  CONTACT_ID,MD5,CONTENT_TYPE,CONTENT  FROM  "+OfflineGroupChatMessageRepository.DAO.getDataSourceBind().table()+"  WHERE  GROUP_ID  IN  (SELECT  CHAT_GROUP_ID  FROM  "+ChatGroupUserRepository.DAO.getDataSourceBind().table()+"  WHERE  CONTACT_ID = ?  AND  IS_DELETED = FALSE)  AND  ID > ?  ORDER  BY  ID  ASC",new  Object[]{userId,extras.get("OFFLINE_GROUP_CHAT_MESSAGES").getLong("LATEST_RECEIVED_ID")}).stream().filter((groupChatMessage) -> {groupChatMessage.addEntry("CREATE_TIME",new  Timestamp(groupChatMessage.getLong("ID"))).addEntry("TRANSPORT_STATE",TransportState.RECEIVED.getValue());  return  true;}).collect(Collectors.toList()) );

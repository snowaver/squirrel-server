...
public  class  OfflineServiceImpl  implements  OfflineService
{
	...
	public  ResponseEntity<String>  search( int  action,long  userId,Map<String,Map<String,Object>>  extras )
	{
		...
		response.addEntry( "CHAT_GROUP_USERS",ChatGroupUser.dao.search("SELECT  ID,IS_DELETED,CREATE_TIME,LAST_MODIFY_TIME,CHAT_GROUP_ID,CONTACT_ID,VCARD  FROM  "+ChatGroupUser.dao.getDataSourceBind().table()+"  WHERE  CHAT_GROUP_ID  IN  (SELECT  CHAT_GROUP_ID  FROM  "+ChatGroupUser.dao.getDataSourceBind().table()+"  WHERE  CONTACT_ID = ?)  AND  LAST_MODIFY_TIME > ?",new  Object[]{userId,new  Timestamp(DateTime.parse(ObjectUtils.cast(extras.get("CHAT_GROUP_USERS").getOrDefault("LAST_MODIFY_TIME","2000-01-01T00:00:00.000Z"),String.class)).getMillis())}) );

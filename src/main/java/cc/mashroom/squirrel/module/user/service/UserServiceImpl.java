...
public  class  UserServiceImpl     implements  UserService
{
	...
	public  ResponseEntity<String>  add( User  user , MultipartFile  portrait )  throws  IOException
	{
		...
		if( User.dao.insert(generatedUserId,"INSERT  INTO  "+User.dao.getDataSourceBind().table()+"  (USERNAME,PASSWORD,NAME,NICKNAME,ROLETYPE)  SELECT  ?,?,?,?,?  FROM  DUAL  WHERE  NOT  EXISTS  (SELECT  ID  FROM  "+User.dao.getDataSourceBind().table()+"  WHERE  USERNAME = ?)",new  Object[]{user.get("username"),new  String(DigestUtils.md5Hex(user.getString("password"))).toUpperCase(),user.get("name"),user.getOrDefault("nickname",user.get("username")),0,user.get("username")}) >= 1 )
		{
	...
	{
		User  user = User.dao.getOne( "SELECT  ID  AS  ID,USERNAME  AS  USERNAME,NAME  AS  NAME,NICKNAME  AS  NICKNAME,ROLETYPE  AS  ROLETYPE  FROM  "+User.dao.getDataSourceBind().table()+"  WHERE  USERNAME = ?  AND  PASSWORD = ?  AND  ROLETYPE = ?",new  Object[]{username,password,roletype} );
		
		String  secretKey= UUID.randomUUID().toString().toUpperCase();
		
		if( user != null )
		{
			CacheFactory.createCache("SESSION_LOCATION_CACHE").update( "MERGE  INTO  SESSION_LOCATION  (USER_ID,CREATE_TIME,CLUSTER_NODE_ID,SECRET_KEY)  VALUES  (?,?,?,?)",new  Object[]{user.getLong("ID"),new  Timestamp(DateTime.now(DateTimeZone.UTC).getMillis()),null,secretKey} );
		}
	...
	{
		User  user = User.dao.getOne( "SELECT  ID,USERNAME,NAME,NICKNAME  FROM  "+User.dao.getDataSourceBind().table()+"  WHERE  ID = ?",new  Object[]{userId} );
	...
	public  ResponseEntity<String>  search( int  action,String  keyword,Map<String,Object>  extras )
	{
		switch( action )
		{
			case  0:
			{
				return  ResponseEntity.ok( JsonUtils.toJson(User.dao.search("SELECT  ID,USERNAME,NAME,NICKNAME  FROM  "+User.dao.getDataSourceBind().table()+"  WHERE  USERNAME = ?",new  Object[]{keyword})) );
			}
			case  1:
			{
				return  ResponseEntity.ok( JsonUtils.toJson(User.dao.search("SELECT  ID,USERNAME,NAME,NICKNAME  FROM  "+User.dao.getDataSourceBind().table()+"  WHERE  ROLETYPE = ?",new  Object[]{0})) );
			}
		}
